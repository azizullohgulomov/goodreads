...
class BookReviewAPITestCase(APITestCase):
    def setUp(self):
        self.user = CustomUser.objects.create(username="azizullohgulomov", first_name="Azizulloh")
    ...
    def test_book_review_detail(self ):
        ...
        self.assertEqual(response.data['book']['isbn'], '1111111')
        self.assertEqual(response.data['book']['id'], self.user.id)
        self.assertEqual(response.data['book']['first_name'], 'Azizulloh')
        self.assertEqual(response.data['book']['username'], 'azizullohgulomov')
    ...
    def test_book_review_list(self):
        ...
        book = Book.objects.create(title="Book1", description="Description1", isbn="1111111")
        br = BookReview.objects.create(book=book, user=self.user, stars_given=5, comment="Very good book")
        br_two = BookReview.objects.create(book=book, user=user_two, stars_given=3, comment="Not bad")
